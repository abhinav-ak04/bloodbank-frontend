...
const BloodBankList = ({ latitude, longitude, bloodGroup }) => {
        ...
        const fetchBloodBanks = async () => {
            try {
                ...
                const response = await axios.get('http://localhost:5000/api/blood-banks', {
                    params: { lat: latitude, lng: longitude, radius: 10, bloodGroup },
    ...
    if (loading) {
        return (
            <div className="text-center py-4">
                <span className="animate-spin inline-block mr-2"><Icons.Clock /></span>
                Loading blood banks...
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4 max-h-96 overflow-y-auto">
            {bloodBanks.length === 0 ? (
                <p className="text-gray-500 text-center">No blood banks found nearby with {bloodGroup} in stock</p>
            ) : (
                bloodBanks.map((bank) => (
                    <div key={bank._id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition">
                        ...
                        <p className="text-sm text-gray-600 mt-1">
                            Stock ({bloodGroup}): {bank.bloodStock[bloodGroup]} units
                        </p>
                        <p className="text-sm text-gray-600 mt-1">
...
const DashboardPage = () => {
    ...
    const getDonationMessage = () => {
        if (!canDonate) return { message: 'You have no blood units available', type: 'error' };
        return { message: "You're eligible to donate now (although 3 months gap is advised)!", type: 'success' };
    };
    ...
    const handleDonation = async () => {
        ...
        try {
            ...
            const updatedUser = {
                ...
                donationHistory: [...(currentUser.donationHistory || []), newDonation],
            };
        ...
        <div className="min-h-screen bg-gray-50">
                    ...
                    <div className="max-w-2xl">
                        ...
                        <div className="flex flex-wrap gap-4 mt-6">
                            ...
                            >
                                {loading ? (
                                    <>
                                        <span className="animate-spin mr-2"><Icons.Clock /></span>
                                        Processing...
                                    </>
                                ) : (
                                    <>
                                        <span className="mr-2"><Icons.Droplet /></span>
                                        I Donated Today
                                    </>
                                )}
                            </button>
            ...
            <div className="container mx-auto px-4 pt-6">
                ...
                >
                    ...
                    >
                        {donationStatus.type === 'success' ? <Icons.Check /> : <Icons.Alert />}
                    </div>
                    <div>
                        <p
                            className={`font-medium ${
                                donationStatus.type === 'success' ? 'text-green-800' : 'text-red-800'
                            }`}
                        >
            ...
            <div className="container mx-auto px-4 py-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-red-500 hover:shadow-md transition group">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                <h3 className="text-gray-500 text-sm font-medium">Blood Group</h3>
                                <p className="text-2xl font-bold text-red-600">{userStats.bloodGroup}</p>
                            </div>
                        </div>
                        <div className="mt-4 pt-4 border-t border-gray-100">
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-500">Universal Recipient</span>
                                <span className="font-medium">{userStats.bloodGroup === 'AB+' ? 'Yes' : 'No'}</span>
                            </div>
                            <div className="flex justify-between items-center mt-1">
                                <span className="text-sm text-gray-500">Universal Donor</span>
                                <span className="font-medium">{userStats.bloodGroup === 'O-' ? 'Yes' : 'No'}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-blue-500 hover:shadow-md transition group">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                <h3 className="text-gray-500 text-sm font-medium">Available Units</h3>
                                <p className="text-2xl font-bold text-gray-800">{userStats.availableUnits}</p>
                            </div>
                        </div>
                        <div className="mt-4 pt-4 border-t border-gray-100">
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-500">Status</span>
                                <span className={`font-medium ${userStats.availableUnits === 0 ? 'text-red-500' : 'text-green-500'}`}>
                                    {userStats.availableUnits === 0 ? 'No units available' : 'Units available'}
                                </span>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-2.5 mt-2">
                                <div
                                    className={`${userStats.availableUnits === 0 ? 'bg-red-600' : 'bg-blue-600'} h-2.5 rounded-full transition-all duration-500`}
                                    style={{ width: `${Math.min(userStats.availableUnits * 20, 100)}%` }}
                                ></div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2">
                        <div className="bg-white rounded-xl shadow-sm p-6 mb-6 hover:shadow-md transition">
                            <h2 className="text-xl font-bold mb-4 flex items-center">
                                <span className="h-5 w-5 mr-2 text-red-600"><Icons.Droplet /></span>
                                Blood Compatibility
                            </h2>
                            {userStats.bloodGroup && compatibilityInfo[userStats.bloodGroup] && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="border border-green-200 rounded-lg p-4 bg-green-50">
                                        <h3 className="text-green-700 font-medium mb-2">Can Donate To</h3>
                                        <div className="flex flex-wrap gap-2">
                                            {compatibilityInfo[userStats.bloodGroup].canDonateTo.map((group) => (
                                                <span key={group} className="bg-white px-3 py-1 rounded-full text-green-600 border border-green-200 font-medium">
                                                    {group}
                                                </span>
                                            ))}
                                        </div>
                                    ...
                                    <div className="border border-blue-200 rounded-lg p-4 bg-blue-50">
                                        <h3 className="text-blue-700 font-medium mb-2">Can Receive From</h3>
                                        <div className="flex flex-wrap gap-2">
                                            {compatibilityInfo[userStats.bloodGroup].canReceiveFrom.map((group) => (
                                                <span key={group} className="bg-white px-3 py-1 rounded-full text-blue-600 border border-blue-200 font-medium">
                                                    {group}
                                                </span>
                                            ))}
                                        </div>
                            ...
                            <div className="mt-6 bg-red-50 rounded-lg p-4 border border-red-200">
                                <h3 className="text-red-700 font-medium mb-2">Did you know?</h3>
                                <p className="text-red-800 text-sm">
                                    {userStats.bloodGroup === 'O-' ? 'Your blood type (O-) is universal and can be given to anyone in an emergency.'
                                        : userStats.bloodGroup === 'AB+' ? 'Your blood type (AB+) means you can receive any type of blood donation.'
                                        : userStats.bloodGroup === 'O+' ? 'Your blood type (O+) is the most common, about 38% of the population has it.'
                                        : userStats.bloodGroup === 'A+' ? 'Your blood type (A+) is the second most common, found in about 34% of the population.'
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-xl shadow-sm p-6 mb-6 hover:shadow-md transition">
                            <h2 className="text-xl font-bold mb-4 flex items-center">
                                <span className="h-5 w-5 mr-2 text-red-600"><Icons.MapPin /></span>
                                Nearby Blood Banks
                            </h2>
                            <BloodBankList 
