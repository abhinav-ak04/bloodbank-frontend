...
const LoginPage = () => {
  const { login, isAuthenticated, loading: authLoading, currentUser } = useContext(AuthContext);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      setError('');
      await login(formData.email, formData.password, 'donor'); // Default to 'donor'
    } catch (err) {
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12 bg-gradient-to-b from-white to-red-50">
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-8 border border-gray-100">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Log in to continue your lifesaving journey</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <button
              type="submit"
              className="w-full bg-red-600 text-white py-3 px-6 rounded-lg font-medium hover:bg-red-700 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 shadow-md disabled:opacity-50"
              disabled={loading || authLoading}
            >
              {loading || authLoading ? (
