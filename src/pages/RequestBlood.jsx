...
function RequestBlood() {
        ...
        try {
            ...
            try {
                ...
                const response = await axios.get(
                    `${process.env.REACT_APP_API_URL}/geocoding`,
                    {
                        params: {
                ...
                const formattedAddress = `${addressObj?.locality || ''}, ${
                    addressObj?.city || ''
                } ${addressObj?.postcode || ''}, ${
                ...
                setUserData((prev) => ({
                    ...prev,
                    address: `Lat: ${newPosition.latitude.toFixed(
                        4
                    )}, Lng: ${newPosition.longitude.toFixed(4)}`,
                }));
            }
        } catch (err) {
            setError(
    ...
    const handleSubmit = (e) => {
        ...
        navigate(
            `/blood-bank-results?lat=${encodeURIComponent(
                position.latitude || ''
            )}&lng=${encodeURIComponent(
                position.longitude || ''
            )}&bloodGroup=${encodeURIComponent(userData.bloodGroup)}`
        );
    };
            ...
            <div className="max-w-md w-full space-y-8 bg-white rounded-2xl shadow-xl p-8">
                <div className="text-center">
                    ...
                    <p className="text-gray-600">
                        Fill in the details to find available blood banks
                    </p>
                </div>
