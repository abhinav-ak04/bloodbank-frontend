...
const Chatbot = () => {
  ...
  const commonTopics = [
    ...
    { label: 'Benefits', query: 'What are the benefits of donating blood?' },
  ];

  // Fallback responses when server connection fails
  const fallbackResponses = {
    greeting: 'Welcome to Blood Bank Assistant! How can I help you today?',
    donation: 'To donate blood, you generally need to be at least 17 years old, weigh at least 110 pounds, and be in good health. Would you like to know more?',
    eligibility: 'General eligibility requirements include being at least 17 years old, weighing at least 110 pounds, being in good health, and not having any recent tattoos or piercings in the last 12 months.',
    location: 'To find a blood bank near you, I would normally search our database. Since we\'re currently offline, please visit our website or call our hotline at (555) 123-4567 for location information.',
    benefits: 'Donating blood helps save lives, provides a free mini health screening, and may reduce the risk of heart disease by reducing iron stores.',
    default: 'I apologize, but I\'m currently operating in offline mode with limited functionality. For more specific information, please try again later when our connection is restored.',
  };
  ...
  const getFallbackResponse = (message) => {
    ...
    } else if (msg.match(/\b(requirements|eligible|eligibility|qualify|can i donate)\b/)) {
      return fallbackResponses.eligibility;
    } else if (msg.match(/\b(blood bank|location|where|nearby|center|find|closest)\b/)) {
      return fallbackResponses.location;
    } else if (msg.match(/\b(benefits|advantage|good|why donate|reason)\b/)) {
        ...
        <div className="w-96 bg-white rounded-2xl shadow-2xl border border-gray-100 flex flex-col h-[34rem] transition-all duration-300">
          <div className="bg-gradient-to-r from-red-600 to-red-700 text-white p-4 rounded-t-2xl flex justify-between items-center">
            <div className="flex items-center gap-2">
              <h3 className="text-xl font-semibold tracking-tight">Blood Bank Assistant</h3>
              {connectionStatus !== 'connected' && (
                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-800 text-white">
          ...
          <div className="flex-1 p-4 overflow-y-auto bg-gradient-to-b from-white to-gray-50">
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`mb-4 max-w-[85%] ${msg.sender === 'user' ? 'ml-auto text-right' : 'mr-auto text-left'}`}
              >
                <div
